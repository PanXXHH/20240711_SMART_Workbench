...
def 整理文件夹(path: str, config: dict):
    ...
    # 按照文件夹的最后修改时间进行排序
    workspace_folders.sort(key=lambda folder: os.path.getmtime(folder))

    print(workspace_folders)
    return

    # 将获取到的文件夹按名称进行降序排序
    workspace_folders.sort(reverse=True)

    # 如果文件夹数量小于或等于配置中指定的上限，则不进行操作
    folder_limit = config["limit"]
    if len(workspace_folders) <= folder_limit:
        sg.popup("当前文件夹不需要整理", font="微软雅黑")
        return
    
    # 从已排序的文件夹列表中，取出超过限制数量的文件夹，进行整理
    folders_to_move = workspace_folders[folder_limit:]
